...
from bot.misc import exchange, Exchange, CURRENCY
...
async def amount_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        text=f"Currency pair <b>{first_currency}/{second_currency}</b> at the rate: <b>{exchange_result['exchange_rate']}</b>\n\n"
             f"<b>{first_currency} to {second_currency} (amount {amount} {CURRENCY[first_currency]}): {exchange_result['converted_amount']} {CURRENCY[second_currency]}</b>",
        parse_mode="HTML")
    await state.clear()
=======
from aiogram import Router, F
